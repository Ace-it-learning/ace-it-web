#!/usr/bin/env python3
"""
Unified batch seed script for all CityU JUPAS programmes.
Reads from cityu_programmes.json and upserts all programmes to Cosmos DB.

Usage:
    cd backend && python scripts/cityu/seed_cityu_all.py
    cd backend && python scripts/cityu/seed_cityu_all.py --dry-run  # Validate only, no upsert
    cd backend && python scripts/cityu/seed_cityu_all.py --codes JS1007,JS1012  # Seed specific codes only
"""
...
def main():
    parser = argparse.ArgumentParser(description="Seed CityU JUPAS programmes to Cosmos DB")
    parser.add_argument("--dry-run", action="store_true", help="Validate only, no upsert")
    parser.add_argument("--codes", type=str, help="Comma-separated list of programme codes to seed (e.g., JS1007,JS1012)")
    ...
    print(f"[Start] Processing {len(programmes)} CityU programmes...")
