#!/usr/bin/env python3
"""
Unified batch seed script for all HKBU JUPAS programmes.
Reads from hkbu_programmes.json and upserts all programmes to Cosmos DB.

Usage:
    cd backend && python scripts/hkbu/seed_hkbu_all.py
    cd backend && python scripts/hkbu/seed_hkbu_all.py --dry-run  # Validate only, no upsert
    cd backend && python scripts/hkbu/seed_hkbu_all.py --codes JS2020,JS2110  # Seed specific codes only
"""
...
def main():
    parser = argparse.ArgumentParser(description="Seed HKBU JUPAS programmes to Cosmos DB")
    parser.add_argument("--dry-run", action="store_true", help="Validate only, no upsert")
    parser.add_argument("--codes", type=str, help="Comma-separated list of programme codes to seed (e.g., JS2020,JS2110)")
    ...
    print(f"[Start] Processing {len(programmes)} HKBU programmes...")
