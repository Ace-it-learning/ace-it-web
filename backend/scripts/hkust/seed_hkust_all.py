#!/usr/bin/env python3
"""
Unified batch seed script for all HKUST JUPAS programmes.
Reads from hkust_programmes.json and upserts all programmes to Cosmos DB.

Usage:
    cd backend && python scripts/hkust/seed_hkust_all.py
    cd backend && python scripts/hkust/seed_hkust_all.py --dry-run  # Validate only, no upsert
    cd backend && python scripts/hkust/seed_hkust_all.py --codes JS5101,JS5102  # Seed specific codes only
"""
...
def build_admission_section(prog, lang):
    ...
    if lang == "en":
        content = [
            ...
            (f" Subject weighting: {prog['subjectWeighting']}." if prog.get("subjectWeighting") and prog["subjectWeighting"] != "N/A" else ""),
            f"**Highest Attainable Score**: {prog.get('highestAttainable', 'N/A')}.",
            f"**Median Admission Score**: {prog['median']} (2025 JUPAS data).",
            f"**Lower Quartile**: {prog['band_a']} (2025 JUPAS data).",
        ]
        
        # Preferred HKDSE Subjects
        preferred = prog.get("preferredSubjects", "")
        if preferred and preferred != "N/A":
            content.append(f"**Preferred HKDSE Subjects**: {preferred}.")
        ...
        if core:
            ...
            content.append("**Core Subjects Minimum Level**: Chinese Language 3, English Language 3, Mathematics (Compulsory Part) 3, Citizenship and Social Development Attained.")
        ...
        content = [
            f"**入學計分公式**：{formula_zh}。" +
            (f"科目比重：{weighting_zh}。" if weighting_zh and weighting_zh != "N/A" else ""),
            f"**最高可達分數**：{prog.get('highestAttainable', 'N/A')}。",
        ...
        preferred = prog.get("preferredSubjects", "")
        if preferred and preferred != "N/A":
            content.append(f"**文憑試選修科平均成績**：{preferred}。")
        ...
        if core:
            ...
            content.append("**核心科目最低要求**：中國語文第3級、英國語文第3級、數學（必修部分）第3級、公民與社會發展科達標。")
        
        elective = j.get("electiveSubjects", "任何2科達第3級")
...
def main():
    parser = argparse.ArgumentParser(description="Seed HKUST JUPAS programmes to Cosmos DB")
    parser.add_argument("--dry-run", action="store_true", help="Validate only, no upsert")
    parser.add_argument("--codes", type=str, help="Comma-separated list of programme codes to seed (e.g., JS5101,JS5102)")
    ...
    print(f"[Start] Processing {len(programmes)} HKUST programmes...")
