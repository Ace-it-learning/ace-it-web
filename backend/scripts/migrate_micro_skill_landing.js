/**
 * Migrate micro_skill_landing data from Firestore backup JSON to Cosmos DB.
 */
const fs = require('fs');
...
const BACKUP_PATH = path.join(__dirname, '..', '..', 'backups', 'firestore', '2026-05-02_14-01', 'micro_skill_landing.json');

function normalizeDoc(id, raw) {
  const doc = { ...raw };
  doc.id = id;
  doc.pk = 'micro_skill_landing';
...
async function migrate() {
  ...
  const container = await getContainer('micro_skill_landing', '/pk');
