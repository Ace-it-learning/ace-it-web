...
const scenario_001 = {
    ...
    paper: 'Listening',
    subject: 'English',
    type: 'listening_mission', // CRITICAL for dashboard visibility
    is_approved: true,
    is_factory: false,
    factory_template: false,
...
async function patch() {
    const targetId = 'Y6nZkRfwYLTCboRgV3P4'; // The ID found in the dashboard
    console.log(`Patching Target ID: ${targetId}`);
    try {
        await db.collection('question_bank').doc(targetId).set(scenario_001, { merge: true });
        // Also keep our clean ID synchronized
        await db.collection('question_bank').doc('listening_mission_1').set(scenario_001);
        
        console.log("✅ Success: Scenario #001 updated for both Target ID and Reference ID.");
        process.exit(0);
    } catch (error) {
