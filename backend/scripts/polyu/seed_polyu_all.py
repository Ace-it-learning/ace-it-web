#!/usr/bin/env python3
"""
Unified batch seed script for all PolyU JUPAS programmes.
Reads from polyu_programmes.json and upserts all programmes to Cosmos DB.

Usage:
    cd backend && python scripts/polyu/seed_polyu_all.py
    cd backend && python scripts/polyu/seed_polyu_all.py --dry-run  # Validate only, no upsert
    cd backend && python scripts/polyu/seed_polyu_all.py --codes JS3000,JS3060  # Seed specific codes only
"""
...
def build_admission_section(prog, lang):
    ...
    if lang == "en":
        ...
        # Average HKDSE Score with Subject Weighting — PolyU unique metric
        if avg_score and avg_score != "N/A":
            content.append(f"**Average HKDSE Score with Subject Weighting**: {avg_score} (2025 JUPAS data).")
        
        # Admission Score Calculation Mechanism — PolyU unique
        if calc_mechanism and calc_mechanism != "N/A":
            content.append(f"**Admission Score Calculation Mechanism**: {calc_mechanism}.")
        ...
        # Subject weighting note
        if prog.get("subjectWeighting") and prog["subjectWeighting"] != "N/A":
            content.append(f"**Subject Weighting**: {prog['subjectWeighting']}.")
        ...
        if core:
            ...
            content.append("**Core Subjects Minimum Level**: Chinese Language 3, English Language 3, Mathematics (Compulsory Part) 2, Citizenship and Social Development Attained.")
        
        # Elective subjects
        elective = j.get("electiveSubjects", "ANY 2 SUBJECTS at Level 3")
        ...
        if j.get("duration"):
            ...
            content.append("**Duration**: 4 years full-time.")
        if j.get("tuition"):
            content.append(f"**First Year Tuition**: {j['tuition']} (local students).")
        else:
            content.append("**First Year Tuition**: HK$44,500 (local students).")
        
        content.append("**Note**: Subject weightings apply. See \"Weightings\" for details.")
        ...
        formula_zh = formula_zh.replace('Any Best 5 Subjects', '任何最佳五科')
        formula_zh = formula_zh.replace('Any Best 5', '任何最佳五科')
        ...
        formula_zh = formula_zh.replace('plus 6th subject bonus', '另加第6科加分')
        
        content = [
            f"**入學計分公式**：{formula_zh}。",
        ]
        
        # Average HKDSE Score with Subject Weighting — Chinese
        if avg_score and avg_score != "N/A":
            content.append(f"**文憑試成績平均積點（計算科目比重後）**：{avg_score}（2025年聯招數據）。")
        
        # Admission Score Calculation Mechanism — Chinese
        if calc_mechanism and calc_mechanism != "N/A":
            content.append(f"**入學分數計算機制**：{calc_mechanism}。")
        ...
        # Subject weighting note
        if prog.get("subjectWeighting") and prog["subjectWeighting"] != "N/A":
            weighting_zh = prog["subjectWeighting"]
            ...
            content.append(f"**科目比重**：{weighting_zh}。")
        ...
        if core:
            ...
            content.append("**核心科目最低要求**：中國語文第3級、英國語文第3級、數學（必修部分）第2級、公民與社會發展科達標。")
        ...
        if j.get("duration"):
            ...
            content.append("**修讀年期**：4年全日制。")
        if j.get("tuition"):
            tuition_zh = j['tuition'].replace('HK$', '港幣') + '元'
            content.append(f"**首年學費**：{tuition_zh}（本地學生）。")
        else:
            content.append("**首年學費**：港幣44,500元（本地學生）。")
        
        content.append("**備註**：科目比重詳見「比重」按鈕。")
...
def main():
    parser = argparse.ArgumentParser(description="Seed PolyU JUPAS programmes to Cosmos DB")
    parser.add_argument("--dry-run", action="store_true", help="Validate only, no upsert")
    parser.add_argument("--codes", type=str, help="Comma-separated list of programme codes to seed (e.g., JS3000,JS3060)")
    ...
    print(f"[Start] Processing {len(programmes)} PolyU programmes...")
