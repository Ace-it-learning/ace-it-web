...
async function saveQuestions() {
    console.log(`[Upload] Saving ${questions.length} corrected questions to question_bank...`);
    const batch = db.batch();
    
    for (const q of questions) {
        const hashStr = `${q.topic_id}-${q.level}-${q.question.substring(0, 50)}`;
    ...
    await batch.commit();
    console.log(`[Upload] SUCCESS: Saved ${questions.length} questions.`);
}

saveQuestions().then(() => process.exit(0)).catch(err => {
    console.error(err);
    process.exit(1);
});
