...
const AlertModal = ({ isOpen, type = 'info', message, onClose, onRetry }) => {
    ...
    const styles = {
        ...
        network: {
            ...
            text: 'text-amber-800',
            icon: <WifiOff size={32} className="text-amber-600" />,
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
            ...
            {/* Modal Card */}
            <div className={`relative bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200 border-2 ${currentStyle.border}`}>
                <div className="p-6 flex flex-col items-center text-center">
                    {/* Icon Bubble */}
                    {currentStyle.icon && (
                        <div className={`mb-4 p-4 rounded-full ${currentStyle.bg}`}>
                            {currentStyle.icon}
                        </div>
                    )}
                    ...
                    <div className="flex gap-3 w-full">
                        ...
                        <button
                            onClick={onClose}
                            className={`flex-1 py-2.5 px-4 text-white rounded-xl font-bold shadow-lg shadow-gray-200 transition-all active:scale-95 ${currentStyle.btn}`}
                        >
                            {onRetry ? 'Close' : 'OK'}
                        </button>
                    </div>
