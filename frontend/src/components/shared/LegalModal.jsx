...
import { X, FileText, Shield, ShieldAlert } from 'lucide-react';
import { useLanguage } from '../../context/LanguageContext';

const LegalModal = ({ isOpen, onClose, type }) => {
    const { t, language } = useLanguage();

    const isIframeDoc = type === 'terms' || type === 'privacy';
    const iframeSrc =
        type === 'terms' ? `/terms_${language}.html` : type === 'privacy' ? `/privacy_${language}.html` : null;
    ...
    const content = type === 'disclaimer' ? t('legal.disclaimer') : null;

    const Icon = type === 'terms' ? FileText : type === 'privacy' ? Shield : ShieldAlert;
                    ...
                    <div className="bg-white dark:bg-[#1a110a] rounded-3xl shadow-2xl overflow-hidden flex flex-col border border-white/10 h-full max-h-[90vh]">
                        <div className="p-6 border-b border-gray-100 dark:border-white/5 flex items-center justify-between bg-gray-50/50 dark:bg-white/5 flex-shrink-0">
                            <div className="flex items-center gap-3">
                                <div className={`p-2 rounded-xl ${iconClass} dark:bg-white/5`}>
                                    <Icon size={24} />
                                </div>
                        ...
                        <div
                            className={`overflow-y-auto custom-scrollbar bg-white dark:bg-[#1a110a] flex-1 ${isIframeDoc ? 'p-0' : 'p-8 md:p-12'}`}
                        >
                            {isIframeDoc ? (
                                <iframe
                                    src={iframeSrc}
