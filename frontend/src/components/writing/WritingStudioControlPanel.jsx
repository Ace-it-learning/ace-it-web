...
const WritingStudioControlPanel = ({ 
    ...
    // If in Mock mode, force Spark tab and never switch
    useEffect(() => {
        if (isMock) setActiveTab('spark');
    }, [isMock]);

    return (
        <div className="flex flex-col h-full bg-white border-l border-slate-100">
            {/* Tab Navigation - Hidden in Mock Mode to simplify UI */}
            {!isMock && (
            ...
            {isMock && (
                <div className="bg-indigo-600 px-6 py-4 flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-black text-white uppercase tracking-widest leading-none">Exam Integrity Active</span>
                </div>
            )}
                    ...
                    {activeTab === 'spark' && (
                        ...
                        >
                            <div className="flex items-center gap-3 mb-6">
                                ...
                                <div>
                                    <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest leading-none">The Spark</h4>
                                    <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-tight">Scratchpad & Outlining</p>
                                </div>
                            </div>
                            ...
                            <div className="flex-1 min-h-[400px] flex flex-col">
                                <textarea
                                    ...
                                    placeholder="Jot down your mind-map or outline here..."
                                    className="flex-1 w-full bg-amber-50/40 border-2 border-amber-100/50 rounded-[2.5rem] p-8 text-sm font-bold text-slate-700 placeholder:text-amber-200 focus:outline-none focus:border-amber-300 transition-all shadow-inner resize-none leading-relaxed"
                                />
                            </div>
                            
                            <div className="mt-6 p-5 bg-slate-900 rounded-[2rem] border border-slate-800 shadow-xl">
                                <p className="text-[11px] font-bold text-slate-300 leading-snug italic flex items-start gap-3">
                                    <Star size={14} className="text-amber-400 shrink-0 mt-0.5" />
                                    "Level 5** writers spend 10-15 minutes planning their structure before drafting. Use this space for your roadmap."
                                </p>
                            </div>
                        </motion.div>
                    )}
                        ...
                        >
                            <div className="flex items-center justify-between mb-8">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h4 className="text-sm font-black text-slate-900 uppercase tracking-widest leading-none">AI Marker</h4>
                                        <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-tight">Real-time Analysis</p>
                                    </div>
                                </div>
                                <button
                            ...
                            {reviewData ? (
                                reviewData.status === 'short_content' ? (
                                    <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-rose-50/50 rounded-[2.5rem] border-2 border-dashed border-rose-100">
                                        ...
                                        <h5 className="text-[10px] font-black text-rose-900 uppercase tracking-widest mb-2">Draft Too Short</h5>
                                    ...
                                    <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-slate-50 rounded-[2.5rem]">
                                        ...
                                        <h5 className="text-[10px] font-black text-slate-900 uppercase tracking-widest mb-2">Analysis Hub Busy</h5>
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="grid grid-cols-3 gap-2">
                                            {Object.entries(reviewData.clo_status || {}).map(([key, val]) => (
                                                <div key={key} className="bg-slate-50 border border-slate-100 rounded-xl p-3 flex flex-col items-center">
                                                    <span className="text-[8px] font-black text-slate-400 uppercase mb-1">{key[0]}</span>
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            {reviewData.paragraph_analysis?.map((item, idx) => (
                                                <div key={idx} className={`p-4 rounded-2xl border ${item.type === 'strength' ? 'bg-emerald-50 border-emerald-100' : 'bg-rose-50 border-rose-100'}`}>
                                                    <div className="flex items-center gap-2 mb-2">
                                                        ...
                                                        <span className={`text-[10px] font-black uppercase tracking-widest ${item.type === 'strength' ? 'text-emerald-700' : 'text-rose-700'}`}>
                                            ...
                                            <div className="space-y-3">
                                                <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Vocabulary Upgrades</h5>
                                                {reviewData.vocabulary_upgrades?.map((item, idx) => (
                                                    <div key={idx} className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                                                        <span className="text-xs font-bold text-slate-400 line-through shrink-0">{item.original}</span>
