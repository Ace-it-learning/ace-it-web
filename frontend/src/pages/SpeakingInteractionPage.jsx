...
const SpeakingInteractionPage = () => {
    ...
    const roadmapTopic = location.state?.topic || 'General Academic Discussion';
    ...
    const [isEasyMode, setIsEasyMode] = useState(!location.state?.mode?.includes('mock')); // Auto-ON for Practice, OFF for Mock
    ...
    // 1. Initialize Quest Data (Dynamic Topic from Roadmap)
    useEffect(() => {
        console.log("[SpeakingQuest] Initializing with topic:", roadmapTopic);

        // Generate dynamic discussion data based on roadmap topic
        const questData = {
            id: `quest_${Date.now()}`,
            title: roadmapTopic,
    ...
    const finishExam = async () => {
        ...
        // Grade logic with Quest context
        const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:3001';
        try {
            navigate('/speaking/result', { state: { loading: true, isQuest: true, taskId } }); // Optimistic nav
            ...
            const res = await fetch(`${API_URL}/api/speaking/grade`, {
                ...
                body: JSON.stringify({
                    ...
                    studentName: user?.displayName || 'Candidate D',
                    uid: user?.uid,
                    isQuest: true,
                    taskId: taskId
                }),
            ...
            if (res.ok) {
                ...
                navigate('/speaking/result', { state: { result, examData, isQuest: true, taskId } });
            } else {
            ...
            navigate('/speaking/result', { state: { error: e.message, isQuest: true } });
        }
    };
    ...
    if (!examData) return <div className="h-screen bg-indigo-50 flex items-center justify-center text-indigo-600 font-bold animate-pulse">Loading Speaking Quest...</div>;
    ...
    return (
        <div className="h-screen w-full relative overflow-hidden flex flex-col">
                ...
                <div className="h-[40%] w-full bg-[#fdfaf5] border-b-8 border-[#d4a373] relative">
                    ...
                    <div className="absolute top-[15%] left-1/2 -translate-x-1/2 w-[60%] h-[70%] bg-[#2d3a3a] border-[12px] border-[#5d4037] rounded shadow-2xl flex items-center justify-center p-8">
                        <div className="text-white/20 font-handwriting select-none transform -rotate-1 text-center">
                            <h2 className="text-3xl font-bold mb-2 opacity-40">SPEAKING INTERACTION QUEST</h2>
            ...
            `}} />
            {/* DEBUG LOGGER */}
            {(import.meta.env.DEV || user?.email === 'fungtam@gmail.com') && (
